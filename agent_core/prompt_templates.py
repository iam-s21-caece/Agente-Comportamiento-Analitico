...
1) CVE-2026-33871 — Netty HTTP/2 CONTINUATION flood (DoS).
   ...
   - Evidencia VOLUMÉTRICA por UMBRALES sostenidos vs baseline. FIRMA REAL validada \
empíricamente: es CPU-BOUND — la CPU del host va a ~100% y la CPU del PROCESO JVM \
se dispara (decenas de % vs ~1% en reposo). NO crecen ni los threads (el pool \
event-loop de Netty es fijo, ~40), ni la memoria (RES plana), ni las tasks: si \
mencionás la firma, hablá de saturación de CPU, NO de threads/memoria (no se mueven).
   - Para esta CVE usá `get_host_metrics_snapshot`, `get_container_metrics_snapshot` \
y `run_deterministic_check('dos_thresholds')`.
   - ATRIBUCIÓN DE ORIGEN: llamá a `get_connection_snapshot` para identificar la(s) \
IP(s) con concurrencia anómala hacia Keycloak e incluilas en la evidencia y la \
narrativa (análogo a nombrar los usuarios en el OIDC). Si el resultado trae \
`masked: true`, la IP top es loopback/privada (enmascarada por NAT/proxy/túnel): \
reportá la concurrencia y la IP aparente, PERO aclarando que la IP real del atacante \
externo está enmascarada por la topología. No afirmes una IP externa que no observaste.
