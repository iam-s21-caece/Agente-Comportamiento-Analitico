...
class Settings(BaseSettings):
    ...
    # Precios de DeepSeek en USD por 1M de tokens (para la métrica de costo).
    # Configurables: actualizalos si cambia la tarifa o el modelo. Referencia
    # deepseek-chat: input cache-miss 0.14, cache-hit 0.028, output 0.28.
    deepseek_price_input_miss_per_1m: float = Field(default=0.14)
    deepseek_price_input_hit_per_1m: float = Field(default=0.028)
    deepseek_price_output_per_1m: float = Field(default=0.28)
    ...
    # Panel de diagnóstico: snapshot de mensajes específicos/accionables del agente
    # (eventos, JVM, baseline, dashboard, LLM). Se sirve al dashboard.
    diagnostics_path: Path = Field(default=AGENT_PACKAGE_DIR / "data" / "diagnostics.json")
