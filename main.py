...
# Atribución de origen del DoS: muestreamos conexiones cuando la CPU supera este
# umbral (el flood ya está saturando) y retenemos el pico esta cantidad de
# segundos, para atribuir aunque el ataque cese al momento del análisis.
_CONN_CAPTURE_CPU = 80.0
_CONN_RETAIN_SECONDS = 45.0
...
class SecurityAgent:
    ...
    def __init__(self, settings: Settings, use_llm: bool = True) -> None:
        ...
        self.connection_collector = ConnectionCollector(settings)
        ...
        # Pico de conexiones retenido durante la saturación (atribución del DoS).
        self._peak_connections: Any = None
        self._peak_connections_ts: float = 0.0
    ...
    def run_continuous(self) -> None:
        ...
        self.events_collector.self_check()
        self._startup_diagnostics()
        # Reporte de validación al arrancar: si ya hay alertas persistidas de
        # corridas anteriores, el panel de Resultados se llena de entrada (sin
        # esperar una alerta nueva ni correr un comando manual).
        self._refresh_validation_report()
        ...
        try:
            while True:
                ...
                try:
                    ...
                    get_diagnostics(self.settings).report(
                        "cycle",
                        "error",
                        f"Error en un ciclo de monitoreo: {exc}",
                        "El agente continúa; si se repite, revisá conectividad a "
                        "Keycloak/LLM y los logs del contenedor.",
                    )
                time.sleep(interval)
    ...
    def _run_cycle(self, event_window: deque) -> None:
        ...
        # Tiempo de respuesta interno: se empieza a contar EN CUANTO la huella se
        # identifica (el check dispara), hasta la emisión de la alerta. Automático,
        ...
        # Estampar métricas observacionales (tiempo de respuesta + consumo) para
        # el panel en vivo del dashboard, que se actualiza con cada alerta.
        self._stamp_metrics(alert, trigger_ts, usage)
    ...
    def _stamp_metrics(self, alert: Alert, trigger_ts: float, usage: Any) -> None:
        """Estampa métricas observacionales del agente sobre la alerta.

        Args:
            alert: alerta a la que se le agregan las métricas.
            trigger_ts: ``time.monotonic()`` del instante en que la huella disparó.
            usage: consumo del análisis (``Usage``) o ``None`` si fue determinista.
        """
        alert.metrics.response_time_s = round(time.monotonic() - trigger_ts, 3)
        if usage is not None:
            alert.metrics.tokens = usage.total_tokens
            alert.metrics.cost_usd = estimate_cost_usd(
                {
    ...
    def _emit(self, alert: Alert) -> None:
        ...
        # Tras persistir la alerta, refrescar el reporte de validación para que el
        # dashboard exponga las métricas completas SIN ningún paso manual. Es
        # barato (sin LLM ni red: relee alerts.jsonl + escenarios) y el cálculo
        # sigue viviendo en validación (el panel solo muestra).
        self._refresh_validation_report()

    def _refresh_validation_report(self) -> None:
        """Regenera el reporte retrospectivo desde las alertas persistidas.

        Aísla la validación del runtime: cualquier fallo se reporta como
        diagnóstico específico y no interrumpe el monitoreo.
        """
        try:
            from validation.report_service import refresh_retrospective_report

            path = refresh_retrospective_report(self.settings)
            if path is None:
                get_diagnostics(self.settings).report(
                    "validation",
                    "warn",
                    "Aún no hay resultados de validación: ninguna alerta mapea a un "
                    "escenario conocido todavía.",
                    "Ejecutá una prueba (OIDC o DoS) para que el agente alerte; el "
                    "panel de Resultados se llena solo con la próxima alerta.",
                )
            else:
                get_diagnostics(self.settings).report(
                    "validation",
                    "ok",
                    "Resultados de validación actualizados (se recalculan con cada "
                    "alerta).",
                )
        except Exception as exc:  # noqa: BLE001 - la validación no debe tumbar el loop
            logger.error(
                "No se pudo refrescar el reporte de validación",
                extra={"error": str(exc)},
            )
            get_diagnostics(self.settings).report(
                "validation",
                "warn",
                f"No se pudo generar el reporte de validación: {exc}",
                "Revisá VALIDATION_REPORTS_DIR (debe ser escribible) y los YAML de "
    ...
    def _prime_inputs(self, context: AgentContext, event_window: deque | None = None) -> None:
        ...
        # Atribución de origen EN CALIENTE: el flood se autolimita en ~1 min, así
        # que si esperamos a que el check confirme el sostenimiento y arranque el
        # ReAct, las conexiones ya cerraron. Por eso muestreamos las conexiones
        # apenas la CPU se dispara (aunque falte para confirmar) y RETENEMOS el
        # pico, para poder atribuir el origen aunque el ataque cese al analizar.
        conn_active: int | None = None
        if host_snapshot.cpu_percent >= _CONN_CAPTURE_CPU:
            snap = self.connection_collector.collect()
            conn_active = snap.total_connections
            if snap.total_connections > 0:
                self._peak_connections = snap
                self._peak_connections_ts = time.time()
        ...
        logger.info(
            ...
            extra={
                ...
                # Concurrencia hacia Keycloak (solo se mide con CPU elevada).
                "conexiones_a_keycloak": conn_active,
            },
        )
        events = fresh_events
    ...
    def _state_summary(self, context: AgentContext) -> str:
        ...
        # Atribución de origen del DoS: si el check volumétrico disparó, adjuntamos
        # las conexiones por IP. Preferimos una captura en vivo; si el flood ya
        # cesó al momento del análisis (0 conexiones) pero retuvimos un pico
        # reciente durante la saturación, reportamos ese pico (con su antigüedad).
        if any(r.check_id == "dos_thresholds" and r.triggered for r in results):
            summary["connection_snapshot"] = self._attribution_snapshot()
        return json.dumps(summary, ensure_ascii=False, indent=2, default=str)

    def _attribution_snapshot(self) -> dict:
        """Devuelve el snapshot de conexiones para atribuir el origen del DoS.

        Prefiere una captura en vivo; si está vacía (el flood ya cesó) pero hay un
        pico retenido reciente, devuelve ese pico anotando su antigüedad.

        Returns:
            Dict JSON-serializable del ``ConnectionSnapshot`` elegido.
        """
        live = self.connection_collector.collect()
        peak = self._peak_connections
        age = time.time() - self._peak_connections_ts
        if live.total_connections == 0 and peak is not None and age < _CONN_RETAIN_SECONDS:
            data = peak.model_dump(mode="json")
            data["captured_seconds_ago"] = round(age, 1)
            data["note"] = (
                f"{data.get('note', '')} [Pico capturado {round(age, 1)}s antes: el "
                "flood cesó al momento del análisis (se autolimita en ~1 min); se "
                "reporta la concurrencia máxima observada durante el episodio.]"
            ).strip()
            return data
