...
class RemoteTalker(BaseModel):
    """Un origen remoto con conexiones activas hacia Keycloak."""

    ip: str
    connections: int = 0
    scope: str = "public"  # "loopback" | "private" | "public"


class ConnectionSnapshot(BaseModel):
    """Snapshot de conexiones TCP hacia el puerto de Keycloak, por IP de origen.

    Sirve para ATRIBUIR el origen de un flood volumétrico (CVE-2026-33871): la IP
    con concurrencia anómala hacia el puerto de Keycloak, correlacionable con el
    instante en que las métricas se saturan.
    """
    ...
    unique_ips: int = 0
    top_talkers: list[RemoteTalker] = Field(default_factory=list)
    # True si el top-talker es loopback/privado: la IP real quedó enmascarada por
    # NAT/proxy/túnel (la concurrencia sigue siendo real y atribuible al episodio).
    masked: bool = False
    note: str = ""
    error: str | None = None
