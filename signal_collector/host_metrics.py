    ...
    def self_check(self) -> None:
        ...
        if snapshot.jvm is None:
            ...
            get_diagnostics().report(
                "jvm",
                "warn",
                "JVM de Keycloak NO detectada — el DoS pierde su firma (CPU de la JVM)",
                f"Ajustá JVM_CMDLINE_MATCH (actual: '{self.settings.jvm_cmdline_match}') "
                "y verificá que el contenedor corra con pid: host.",
            )
        else:
            jvm = snapshot.jvm
            get_diagnostics().report(
                "jvm", "ok", f"JVM de Keycloak detectada (pid {jvm.pid})"
            )
            logger.info(
