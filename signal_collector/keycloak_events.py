...
class KeycloakEventsCollector:
    ...
    def self_check(self) -> None:
        ...
        if config is None:
            ...
            get_diagnostics().report(
                "keycloak_events",
                "error",
                "No se pudo leer la config de eventos de Keycloak",
                "Revisá credenciales admin, realm y conectividad "
                "(KEYCLOAK_BASE_URL / ADMIN_USER / ADMIN_PASSWORD).",
            )
            return
        ...
        if not enabled:
            ...
            diag.report(
                "keycloak_events",
                "error",
                "Eventos NO habilitados en Keycloak — CVE-2023-0264 indetectable",
                "Realm Settings → Events → activá 'Save events' y reintentá el ataque.",
            )
        elif not in_24h:
            logger.warning(
            ...
            diag.report(
                "keycloak_events",
                "warn",
                "Eventos habilitados pero 0 en las últimas 24h",
                "Verificá que la prueba genere LOGIN/CODE_TO_TOKEN en el realm "
                "monitoreado y que el admin tenga permiso view-events.",
            )
        else:
            diag.report(
                "keycloak_events",
                "ok",
                f"Eventos habilitados ({len(in_24h)} en las últimas 24h)",
            )
